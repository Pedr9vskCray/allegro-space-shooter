...
#include "data/rand_nums.h"
#include "data/collide.h"
#include "data/must_init.h"
...
ALLEGRO_AUDIO_STREAM* music_stream_init(){
    ALLEGRO_AUDIO_STREAM* music = al_load_audio_stream("assets/4thtrain.opus", 2, 2048);
...
void music_stream_deinit(ALLEGRO_AUDIO_STREAM* music){
...
#define ALIEN_BIGALIEN_W ALIEN_W[2]
#define ALIEN_BIGALIEN_H ALIEN_H[2]
...
void sprites_init()
{
    ...
    sprites.alien[2] = sprite_grab(0, 21, ALIEN_BIGALIEN_W, ALIEN_BIGALIEN_H);
...
{
    if(!spark)
        al_play_sample(sample_explode[randint(0, 2)], 0.75, 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
    ...
    for(int i = 0; i < SHOTS_N; i++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                shots[i].dx = randint(-2, 2);
                shots[i].dy = randint(-2, 2);
            }
...
bool shots_collide(bool ship, int x, int y, int w, int h)
{
    for(int i = 0; i < SHOTS_N; i++)
    {
        ...
        if(collision_check(x, y, x+w, y+h, shots[i].x, shots[i].y, shots[i].x+sw, shots[i].y+sh))
        {
...
void aliens_update()
{
    int new_quota =
        ...
        : randint(2, 4)
    ;
    int new_x = randint(10, BUFFER_W-50);
    ...
    for(int i = 0; i < ALIENS_N; i++)
    {
        if(!aliens[i].used)
        {
            if(new_quota > 0)
            {
                ...
                aliens[i].y = randint(-40, -30);
                aliens[i].type = randint(0, ALIEN_TYPE_N);
                aliens[i].shot_timer = randint(1, 99);
                aliens[i].blink = 0;
                aliens[i].used = true;
...
STAR* init_stars_array(){
    STAR* stars = (STAR*)malloc(STARS_N*sizeof(STAR));
    assert(stars);
    return stars;
}

void stars_preinit(STAR stars[], int n){
    for(int i = 0; i < n; i++){
        stars[i].next = NULL;
        stars[i].prev = NULL;
    }
...
void stars_init(STAR* stars){
    ...
    for(int i = 0; i < STARS_N; i++){
        if (i == 0){
            ...
            stars[i].prev = &stars[i-1];
            stars[i].x = stars[i].prev->x + 2;
        }
        else {
            stars[i].next = &stars[i+1];
            stars[i].prev = &stars[i-1];
        ...
        stars[i].y = randfloat(0, BUFFER_H);
        stars[i].speed = randfloat(0.1,1);
    }
}
...
void stars_draw(STAR* stars)
{
    for(int i = 0; i < STARS_N; i++)
    {
        ...
        al_draw_pixel(stars[i].x, stars[i].y, al_map_rgb_f(l,l,l));
    }
}
...
int main()
{
    ...
    ALLEGRO_AUDIO_STREAM* music = music_stream_init();
    ...
    audio_deinit();
    disp_deinit();
    music_stream_deinit(music);
    al_destroy_timer(timer);
    al_destroy_event_queue(queue);

    FILE* arqv = fopen("score.txt", "at");
    if (arqv == NULL){
        ...
        float time_taken = ((float)stop_time - start_time)/CLOCKS_PER_SEC;

        printf("debug-time -> %.2f // debug-score -> %d", time_taken,score);

        fprintf(arqv, "Time -> %.2f seconds ; Score -> %d points\n", time_taken,score);
